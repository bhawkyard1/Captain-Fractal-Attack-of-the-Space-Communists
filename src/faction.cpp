...
void faction::updateEconomy(const float _dt)
{
	...
	if( !prob(64) or losingWealthTooFast )
	{
	...
	m_aggression *= clamp(0.9999f + m_wealthDT / 4.0f, 0.0f, 1.00001f);
...
void faction::updateDeployment(const std::vector<faction> &_rivals)
{
	...
	/*std::cout << m_identifier << " POWER BALANCES: F " << powerBalance[2] << ", N " << powerBalance[1] << ", E " << powerBalance[0] <<
