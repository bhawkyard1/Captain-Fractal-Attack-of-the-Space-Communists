...
void universe::update(float _dt)
{
    ...
    if(!g_DEV_MODE and m_ply.getHealth() <= 0.0f and !g_GAME_OVER)
    {
        for(int p = 0; p < rand()%5 + 10; ++p)
        {
            ...
            pos += m_ply.getPos();
            addpfx(pos, m_ply.getVel(), rand()%50 + 20, rand()%50 + 8, true, {{255, 200, 20, 255}});
        }
    ...
    for(int i = static_cast<int>(m_shots.size()) - 1; i >= 0; i--)
    {
        ...
        else
        {
            ...
            if(!(rand() % 32)) addpfx(pos, m_shots[i].getVel(), randNum(1.0f, 2.0f), 1.0f, false, m_shots[i].getCol());
        }
    }
        ...
        if(ofscr or m_missiles[i].getHealth() <= 0 or m_missiles[i].detonate())
        {
            if(!ofscr)
            {
                for(int p = 0; p < rand()%2 + 4; p++)
                {
                    ...
                    addpfx(pos, m_missiles[i].getVel(), rand()%50 + 50, rand()%50 + 8, true, {{255, 200, 20, 255}});
                }
        ...
        if((isOffScreen(p,60000.0f) or m_asteroids[i].getHealth() <= 0.0f))
        {
            if(m_asteroids[i].getHealth() <= 0.0f)
            {
                ...
                for(int fx = 0; fx < rand() % 5 + 1; fx++)
                {
                    ...
                    addpfx(pos, m_asteroids[i].getVel(), rand()%20 + 50, rand()%30 + 2, false, {{2, 2, 2, 0}});
    ...
    for(size_t e = 0; e < m_agents.size(); ++e)
    {
        ...
        if(m_agents[e].getGoal() == GOAL_TRADE
                and m_agents[e].getTarget() != nullptr
...
void universe::draw(float _dt)
{
    ...
    for(auto &i : m_particles)
    {
        ...
        {
            m_drawer.addPoint( ipos );
            m_drawer.addColour( col );
            m_drawer.packExtraData( i.getShaderData(), 1 );
        }
    }
...
void universe::drawDebugUI()
{
    ...
    for(auto &f : m_factions)
    {
        if(f.isOrganised())
        {
            ...
            m_drawer.drawText( "AGGRESSION : " + std::to_string(f.getAggression()), "pix", vec2(x, y), false, 1.0f ); y += 20.0f;
...
void universe::checkCollisions()
{
    for(size_t p = 0; p < m_partitions.size(); ++p)
    {
        ...
        for(int l = static_cast<int>(m_partitions[p].m_lasers.size()) - 1; l >= 0; --l)
        {
            ...
            {
                if(neutralityCheck(m_partitions[p].m_lasers[l]->getTeam(), m_partitions[p].m_ships[s]->getTeam()))
                    continue;
                harm = 0;
                ...
                {
                    addpfx(contact, ev, sd * randNum(1.0f, 3.0f), stop * randNum(16.0f, 32.0f) + 12.0f, true, sc);
                    harm = sd;
                ...
                if(harm > 0)
                {
                    ...
                    float xp = clamp( calcAICost(m_partitions[p].m_ships[s]->getClassification()) * harm * 0.0004f, 0.0f, 0.5f);

                    //If last attacker exists, and is another agent
                    if(lastAttacker != nullptr and so.get() != &m_ply)
                    ...
                    addDamagePopup(
                                realDamage,
                                m_partitions[p].m_ships[s]->getTeam(),
                                ep,
                                m_partitions[p].m_ships[s]->getVel() + randVec3(2.0f)
                                );
                    break;
                }
            }
            ...
            for(int r = static_cast<int>(m_partitions[p].m_rocks.size()) - 1; r >= 0; r--)
            {
                ...
                if(lineIntersectSphere(sp, spv, ep, er, &contact))
                {
                    addpfx(contact, ev, sd * randNum(1.0f, 3.0f), stop * randNum(16.0f, 32.0f) + 12.0f, true, {{2, 2, 2, 0}});
            ...
            for(int m = static_cast<int>(m_partitions[p].m_rockets.size()) - 1; m >= 0; m--)
            {
                ...
                if(lineIntersectSphere(sp, spv, ep, er, &contact))
                {
                    addpfx(contact, ev, randNum(3.0f, 8.0f), stop * randNum(16.0f, 32.0f) + 12.0f, true, {{255, 200, 20, 255}});
            ...
            if(!g_GAME_OVER and fabs(dv.m_x) > fabs(dp.m_x) - 32 and fabs(dv.m_y) > fabs(dp.m_y) - 32 and emnityCheck(m_partitions[p].m_lasers[l]->getTeam(), TEAM_PLAYER))
            {
                ...
                if(lineIntersectSphere(sp, spv, dp, m_ply.getRadius(), &contact))
                {
                    ...
                    addpfx(contact, m_ply.getVel(), stop * randNum(16.0f, 32.0f) + 12.0f, randNum(3.0f, 8.0f), true, sc);
                    harm = sd;
        ...
        for(auto &i : m_partitions[p].m_ships)
        {
            ...
            for(auto &j : m_partitions[p].m_ships)
            {
                if(emnityCheck(i->getTeam(), j->getTeam()) and i != j)
                    resolveCollision(static_cast<ship *>(i), static_cast<ship *>(j));
            }

            //Ships vs rocks
            for(auto &j : m_partitions[p].m_rocks)
            {
                if(i->getType() != SHIP_TYPE_MINER)
                    resolveCollision(static_cast<ship *>(i), static_cast<ship *>(j));
            }

            //Ships vs player
            if(emnityCheck(i->getTeam(), TEAM_PLAYER) and !g_GAME_OVER)
                resolveCollision(static_cast<ship *>(i), static_cast<ship *>(&m_ply));
        }

        for(auto &i : m_partitions[p].m_rocks)
        {
            if(!g_GAME_OVER) resolveCollision(i, static_cast<ship *>(&m_ply));
            for(auto &j : m_partitions[p].m_rocks)
                if(i != j)
...
{
    if(_a->hasParent() or _b->hasParent() or _a == _b)
        return;

    float dist = magns(_a->getPos() - _b->getPos());
    if(dist > sqr(_a->getRadius() + _b->getRadius()))
        return;
...
{
    pfx pf(_p, _v, m_vel, _no, _f, _drawExplosion, _col);
        ...
        )
{
    ...
    if(!quit)
    {
        ...
        tempSquad.m_averagePos = _p;
        tempSquad.m_pAveragePos = _p;
        ...
        )
{
    ...
    addpfx(_p, {0.0f, 0.0f, 0.0f}, rand()%20 + 50, newShip.getRadius() * randNum(0.3f, 0.5f), false, {{0.2f, 0.2f, 0.3f, 1.0f}});
...
void universe::dock(int _pidx, ship &_parent, ship &_child)
{
    ...
    if(!_parent.canStoreShips() or _child.isDocked())
        return;
    std::cout << "Docking!\n";
...
{
    ship * lastAttacker = m_agents[_i].getLastAttacker();

    vec3 pos = {randNum(-16.0f,16.0f), randNum(-16.0f,16.0f), 0.0f};
    pos += m_agents[_i].getPos();
    addpfx(pos, m_agents[_i].getVel(), m_agents[_i].getRadius() * randNum(0.5f, 1.0f), m_agents[_i].getRadius() * randNum(1.0f, 1.25f), true, col1to255(m_agents[_i].getCurWeapCol()));
    ...
    float xp = clamp(calcAICost(m_agents[_i].getClassification()) * 0.04f, 0.0f, 5.0f);

    if(lastAttacker != nullptr and lastAttacker != &m_ply)
        lastAttacker->addXP( xp );
    ...
    enemy * lastAttackerEnemy = static_cast<enemy*>(lastAttacker);
    if(lastAttacker != nullptr and lastAttacker != &m_ply and lastAttackerEnemy->getTeam() == TEAM_PLAYER)
    {
        m_factions[ lastAttackerEnemy->getTeam() ].addWealth( calcAICost(m_agents[_i].getClassification() ) );
        if(lastAttackerEnemy->getTeam() == TEAM_PLAYER)
        {
