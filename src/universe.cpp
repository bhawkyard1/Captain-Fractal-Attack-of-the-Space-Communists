...
{
    /*std::cout << "Hey kids, it's time for some stats!\n"
            << "No. of ships : " << m_agents.size() << "\n"
            << "No. of lasers : " << m_shots.size() << "\n"
            << "No. of explosions : " << m_particles.size() << std::endl;*/
...
void universe::draw(float _dt)
{
    ...
    if(m_ply.getShieldGlow() > 1) m_drawer.drawShield(m_ply.getPos(), m_ply.getRadius(), m_time_elapsed / m_ply.getRadius(), m_ply.getShieldGlow() / 255.0f, {0.1f, 0.4f, 1.0f, 1.0f});
    for(auto &i : m_agents)
    {
