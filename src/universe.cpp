    ...
    for(size_t p = 0; p < m_partitions.size(); ++p)
    {
        ...
        for(int l = static_cast<int>(m_partitions[p].m_lasers.size()) - 1; l >= 0; --l)
        {
            ...
            for(int s = static_cast<int>(m_partitions[p].m_ships.size()) - 1; s >= 0; s--)
            {
                ...
                {
                    addpfx(contact, ev, sd * randNum(1.0f, 3.0f), randNum(3.0f, 8.0f), true, sc);
                    harm = sd;
...
void universe::destroyAgent(size_t _i)
{
    ...
    addpfx(pos, m_agents[_i].getVel(), randNum(5.0, 7.0), m_agents[_i].getRadius() * randNum(0.1f, 0.2f), true, col1to255(m_agents[_i].getCurWeapCol()));
