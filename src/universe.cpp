...
{	
  ...
  for(int i = 0; i < BACKGROUND_DOTS; i++)
  {
    ...
    case 2:
      id = "STARDUST_3";
      break;void readVectorEnemy(std::string str, universe * u);
...
void universe::update(float dt)
{
  ...
  for(auto &i : dots)
  {
    i.setWVel(vel);
    i.updatePos(dt);
    if(isOffScreen(i.getPos(), MAX_DIM * BG_DENSITY / ZOOM_LEVEL))
    {
      i.gen(true, cColP);
    }
  }
  ...
  for(auto &i : sparkles)
  {
    ...
    if(isOffScreen(i.getPos(), (MAX_DIM + std::max(w, h)) * BG_DENSITY * i.getZ() / ZOOM_LEVEL))
    {
  ...
  for(int i = enemies.size() - 1; i >= 0; i--)
  {
    ...
    if((isOffScreen(p,30000.0f) and enemies.at(i).getTeam() != TEAM_PLAYER and enemies.at(i).getTeam() != TEAM_PLAYER_MINER) or enemies.at(i).getHealth() <= 0)
    {
      ...
      if(emnityCheck(enemies.at(i).getTeam(), TEAM_PLAYER)) enemy_count--;
