...
void universe::draw(float _dt)
{
  ...
  if(m_ply.getShieldGlow() > 1) m_drawer.drawShield(m_ply.getPos(), m_ply.getRadius(), m_time_elapsed / m_ply.getRadius(), m_ply.getShieldGlow() / 255.0f);
  for(auto &i : m_agents)
  {
    if(i.getShieldGlow() <= 1) continue;
    m_drawer.drawShield(i.getInterpolatedPosition(_dt), i.getRadius(), m_time_elapsed / i.getRadius(), i.getShieldGlow() / 255.0f);
  }
  for(auto &i : m_missiles)
  {
    if(i.getShieldGlow() <= 1) continue;
