        ...
        )
{
    ...
    if(_regen == false)
    {
        setVel( {randNum(-0.1f, 0.1f), randNum(-0.1f, 0.1f), 0.0f} );
        setPos({randNum(-g_WIN_WIDTH / 2.0f, g_WIN_WIDTH / 2.0f), randNum(-g_WIN_HEIGHT / 2.0f, g_WIN_HEIGHT / 2.0f), 0.0f});
        setPPos(getPos());
    }
    else
    {
        ...
        {
            //TOP
            vec = randVec3({-g_HALFWIN.m_x - 100.0f, -g_HALFWIN.m_y - 200.0f, 0.0f}, {g_HALFWIN.m_x + 100.0f, -g_HALFWIN.m_y, 0.0f});
            vec *= g_BG_DENSITY;
            setPos(vec);
            setPPos(getPos());
        }
