...
void renderer_ngl::drawWeaponStats(player *_ply)
{
    ...
    drawText(damageText, "minimal", {fWIN_WIDTH - dim.m_x, fWIN_HEIGHT - 1.4f * dim.m_y}, false, 1.0f);

    std::string noText = "\nLasers " + std::to_string(static_cast<int>(_ply->getCurWeapStat(LASER_COUNT)));
    drawText(noText, "minimal", {fWIN_WIDTH - dim.m_x, fWIN_HEIGHT - 1.2f * dim.m_y}, false, 1.0f);

    std::string spreadText = "\nSpread " + std::to_string(_ply->getCurWeapStat(SPREAD));
    drawText(spreadText, "minimal", {fWIN_WIDTH - dim.m_x, fWIN_HEIGHT - 1.0f * dim.m_y}, false, 1.0f);

    std::string rateText = "\nRate ";
    float rate = 1.0f / _ply->getCurWeapStat(COOLDOWN);
    rateText += std::to_string( static_cast<int>( rate ) );
    drawText(rateText, "minimal", {fWIN_WIDTH - dim.m_x, fWIN_HEIGHT - 0.8f * dim.m_y}, false, 1.0f);
}
