...
void renderer_ngl::drawRect(const vec2 _p, const vec2 _d, const float _ang)
{
  m_transform.setRotation(0.0f, 0.0f, _ang);
    ...
    )
{
  ...
  m_shader->use("text");
  //m_shader->use("debug");
  //m_shader->setRegisteredUniform("inColour", ngl::Vec4(1.0f));
  //If the text is 'in the world' load the mvp.
  //If it to be displayed at a fixed point on the screen, just project it without considering view pos.
  if(_ws)
  {
    loadMatricesToShader();
  }
  ...
  for(int i = 0; i < _text.length(); ++i)
  {
    ...
    w *= _mul;
    h *= _mul;
    drawRect({x, y}, {(float)w, (float)h}, 0.0f);
    x += w;
  }
}
