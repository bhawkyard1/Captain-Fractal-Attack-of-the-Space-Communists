		...
		m_camera()
{
	...
	m_dataFB.addTexture("diffuse", GL_RGBA, GL_RGBA, GL_COLOR_ATTACHMENT0);
	...
	m_nonlitFB.addTexture("background", GL_RGBA, GL_RGBA, GL_COLOR_ATTACHMENT0);
	m_nonlitFB.addTexture("effects", GL_RGBA, GL_RGBA, GL_COLOR_ATTACHMENT1);
	...
	m_litFB.initialise( m_w, m_h );
	m_litFB.addTexture("diffuse", GL_RGBA, GL_RGBA, GL_COLOR_ATTACHMENT0);
	m_litFB.addTexture("depthTex", GL_RED, GL_R16F, GL_COLOR_ATTACHMENT1);
	if(!m_litFB.checkComplete())
		std::cout << "Error! m_litFB creation failed!\n";

	m_smallBackgroundFB.initialise( m_w / AMBIENT_RESOLUTION_DIVIDER, m_w / AMBIENT_RESOLUTION_DIVIDER );
	m_smallBackgroundFB.addTexture("diffuse", GL_RGBA, GL_RGBA, GL_COLOR_ATTACHMENT0);
...
void renderer_ngl::finalise(const float _t, const vec2 _vel)
{
	...
	m_smallBackgroundFB.bindTexture(id, "diffuse", "diffuse", 0);
...
void renderer_ngl::lightingDeferredElements()
{
	m_litFB.bind();
	m_litFB.activeColourAttachments();
}
